#!/usr/bin/env python3
"""Validate a Handbook local-only native/read-only evidence receipt."""
...
from protocol_json import is_absolute_platform_path, is_canonical_branch_ref, load_json
...
def main() -> int:
    ...
    require_string(data.get("consuming_slice"), "consuming_slice")
    ...
    if require_oid(source.get("live_local"), "source.live_local") != commit:
        fail("source.live_local must equal source.commit")
    ...
    environment_host_id = require_string(
        environment.get("host_id"), "environment.host_id"
    )
    ...
    project_path = require_string(
        environment.get("project_path"), "environment.project_path"
    )
    ...
    versions = require_object(
        environment.get("tool_versions"), "environment.tool_versions"
    )
    ...
    artifact_path = require_string(
        evidence.get("artifact_path"), "evidence.artifact_path"
    )
    ...
    print(
        f"VALID evidence receipt {data['evidence_id']} status=EVIDENCE_CLEAN commit={commit}"
    )
    return 0
