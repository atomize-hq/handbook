...
and closeout `5c31eeefb5adf71d75ec3059b1b6947025d2fd6b`. HCM-2.3
planning is explicitly selected; HCM-2.3 implementation remains unauthorized
until the new packet is `CLEAN` and a separate implementation selector is
explicitly selected.
...
| Artifact kind/schema registry | HCM-1.1 supplies the fail-closed stable-role/schema/kind owner boundary. HCM-1.2 adds six exact package-owned structural schema closures and the constitutional capability. HCM-2.1 consumes Project Context. Completed HCM-2.2 additively admits Project Authority `1.1.0`, its typed supporting definitions, and candidate `1.3` while preserving every released definition plus result `1.0` and intent `1.2`. | `BoundaryLanded` for the exact shipped Charter closure and prior registry/meta-validation boundary; `TargetOnly` for generic intake and remaining kind consumers | `handbook-engine` | Preserve exact refs and byte closure; prove generic custom-kind intake only in HCM-2.3 | instance profiles, remaining kind consumers, later Projection engine, generic CLI/SDK operations |
...
| Charter intake coverage | Completed HCM-2.2 implements the versioned 16-coverage `CharterIntakeDefinition`, three equivalent acquisition modes, detailed provenance, result-independent candidate subject, engine-owned result `1.0`, closed final exact-result binding, stable locked replay, and orphan/migration refusal. | `ContractCorrectAndProven` for the exact first-party Charter; `TargetOnly` for generic/custom-kind intake | `handbook-engine` intake/lineage kernel + skill-directed compiler/CLI adapter | Preserve the exact first-party contract; implement generic intake only in HCM-2.3 | generic kind selection, posture kernel, later SDK |
