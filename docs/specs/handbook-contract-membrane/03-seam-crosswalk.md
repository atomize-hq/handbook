...
|---|---|---|---|---|---|
| Canonical artifact identities | HCM-1.3 owns the selected-profile universe through `ResolvedArtifactRegistry`; HCM-1.4 setup/doctor consume one typed closure. HCM-2.1 makes selected `project_context` descriptor-owned. Completed HCM-2.2 makes `project_authority` descriptor-owned through candidate, approval, promotion, and lifecycle currentness. The HCM-2.4 greenfield correction makes selected optional `environment_context` canonical YAML/advisory context, and P3 makes repository-profile-selected `work_specification` canonical YAML the Stage 10 capture/handoff authority. P4's Decision negative-surface/path proof is GREEN and completed; P5 is GREEN and accepted. Accepted P6 makes descriptor-selected `CanonicalArtifacts` the aggregate owner and removes normal-path fixed enum/order/path authority. | `RealPathAdopted` for exact Project Context, Charter, Environment Context, and Stage 10 Work Specification paths and descriptor-selected aggregate flow; `BoundaryLanded` for the broader selected-profile registry | `handbook-engine` profile/artifact kernel | Preserve the adopted owners, accepted Decision/Risk proof, retained exact bytes/identity, and distinct source/render fingerprints; P7 remains separately gated | kind meta-schema, profile schema, generic validation, later Projection proof |
...
| Canonical layout | HCM-2.1 takes `.handbook/project/context.yaml` from the selected Project Context descriptor, and the HCM-2.4 greenfield correction takes `.handbook/project/environment.yaml` from the selected Environment Context descriptor through the trusted safe loader and advisory flow path. Accepted P6 removes normal-path fixed layout/path selection; admitted descriptors now provide canonical paths. | `RealPathAdopted` for the exact Project Context and Environment Context paths and descriptor-selected aggregate layout | `handbook-engine` | Preserve descriptor-owned paths, trusted repository-relative safety, and exact admitted identity | artifact registry, trusted repo-relative paths, later Projection proof |
| Structured authoring | HCM-2.1 makes Project Context canonical YAML the only selected editable truth. Completed HCM-2.2 does the same for Charter. HCM-2.4 makes Environment Context canonical YAML through the generic intake/candidate/promotion writer and derives Markdown only as a view; P3 does likewise for Stage 10 Work Specification. Legacy Environment Inventory Markdown has no authority or readiness influence. Decision/Risk exact mutation/promotion paths are GREEN, and P4's separate negative-surface/path proof is GREEN and completed. Accepted P6 removes aggregate bridge/fixed authority while preserving these owners. | `ContractCorrectAndProven` for exact Project Context, Charter, Environment Context, and Stage 10 Work Specification paths and their aggregate descriptor-selected use | `handbook-engine` plus `handbook-pipeline` for the exact Stage 10 path | Preserve the four canonical owners, their accepted Decision/Risk proofs, and renderer-only derived views | remaining artifact-kind behavior, later Projections |
...
| Flow resolver | Accepted P6 replaces the HCM-2.1/HCM-2.2 bridges and mixed fixed collection with one descriptor-selected `CanonicalArtifacts` collection. It retains exact canonical bytes and admitted instance/kind/path identity, keeps source and rendered fingerprints distinct, and dispatches only the three existing first-party fixed renderers. Charter authority, Project Context legacy-Markdown non-influence, and Environment Context advisory omission/reporting remain GREEN. | `ContractCorrectAndProven` for exact Project Context, Charter, and Environment Context descriptor-selected flow; `BoundaryLanded` for the reduced scope | `handbook-flow` | Preserve descriptor-selected ordering, disposition, summaries, budgets, fixtures, blockers, refusals, and fingerprint domains; do not add generic configured renderer execution | profile kernel, Resolution envelope, SDK DTOs |
