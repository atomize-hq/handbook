...
| Structured authoring | HCM-2.1 makes Project Context canonical YAML the only selected editable truth. Completed HCM-2.2 does the same for Charter. HCM-2.4 makes Environment Context canonical YAML through the generic intake/candidate/promotion writer and derives Markdown only as a view; P3 does likewise for Stage 10 Work Specification. Legacy Environment Inventory Markdown has no authority or readiness influence. Decision/Risk exact mutation/promotion paths are GREEN; P4's separate negative-surface/path proof remains open. | `ContractCorrectAndProven` for exact Project Context, Charter, Environment Context, and Stage 10 Work Specification paths; `UsefulPrecursor` for the still-open P4 negative-surface/path proof | `handbook-engine` plus `handbook-pipeline` for the exact Stage 10 path | Preserve the four canonical owners; preserve their accepted positive paths and complete only P4's separately authorized negative-surface/path proof | remaining artifact-kind behavior, later Projections, aggregate bridge deletion gate |
