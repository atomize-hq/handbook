...
| Flow resolver | Accepted P6 replaces the HCM-2.1/HCM-2.2 bridges and mixed fixed collection with one descriptor-selected `CanonicalArtifacts` collection; propagates exact admitted instance ID, kind ref, label, path, bytes, and distinct source/render fingerprints; and removes normal-path fixed-kind authority. P7 replays the complete packet/workspace wall and exact absence scan. | `ContractCorrectAndProven` for the Phase 2 aggregate under the P7 CLEAN condition | `handbook-flow` | Preserve Charter, Project Context, Environment Context, packet, budget, fixture, blocker, refusal, ordering, and fingerprint behavior; stop before HCM-3.x | profile kernel, Resolution envelope, SDK DTOs |
