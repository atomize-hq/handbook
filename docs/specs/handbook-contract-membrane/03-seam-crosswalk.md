...
and closeout `5c31eeefb5adf71d75ec3059b1b6947025d2fd6b`. The
...
classified, committed, or used to close `PG-KIND-02`; staging is also
forbidden.
In order, HCM-2.3 still requires a different-fresh CLEAN documentation review,
explicit operator authorization of all sixteen CRITICAL production surfaces, a
bounded implementation selector, `atomic-displaced-basis-v1` implementation,
...
classification, `PG-KIND-02` closure, or primary commit. HCM-2.4 and later work
remain unauthorized.
...
|---|---|---|---|---|---|
| Canonical artifact identities | `CanonicalArtifactKind` remains the fixed pre-membrane Markdown product projection for unmigrated sibling seams. HCM-1.3 owns the selected-profile universe through `ResolvedArtifactRegistry`; HCM-1.4 setup/doctor consume one typed closure. HCM-2.1 makes selected `project_context` descriptor-owned. Completed HCM-2.2 makes `project_authority` descriptor-owned through candidate `1.3`, exact result authority, approval, promotion, and lifecycle currentness. | `RealPathAdopted` for exact Project Context and first-party Charter; `BoundaryLanded` for the broader selected-profile registry/setup/doctor boundary; `UsefulPrecursor` for unchanged fixed siblings | `handbook-engine` profile/artifact kernel | Preserve both selected owners and remove mixed-family bridges only after separately reviewed cutovers | kind meta-schema, profile schema, generic validation, remaining layout/flow/content authority |
| Artifact kind/schema registry | HCM-1.1 supplies the fail-closed stable-role/schema/kind owner boundary. HCM-1.2 adds six exact package-owned structural schema closures and the constitutional capability. HCM-2.1 consumes Project Context. Completed HCM-2.2 additively admits Project Authority `1.1.0`, its typed supporting definitions, and candidate `1.3` while preserving every released definition plus result `1.0` and intent `1.2`. HCM-2.3 documentation describes one repository-selected registry-brief kind/schema/intake closure, but current production still uses known non-authoritative verify-then-unconditional-rename publication. | `TargetOnly` for the current exact HCM-2.3 registry-brief subject and remaining generic intake/kind consumers; `BoundaryLanded` for the exact shipped Charter closure and prior registry/meta-validation boundary | `handbook-engine` | Preserve the current exact registry-brief `TargetOnly` baseline and the frozen future `TargetOnly -> RealPathAdopted` diff. Before that diff, `PG-KIND-02` closure, or primary commit: obtain different-fresh CLEAN documentation review, operator authorization of all sixteen CRITICAL surfaces, a bounded selector, `atomic-displaced-basis-v1` implementation, re-completed Checkpoint C/full proof, GitNexus change detection, and different-fresh complete implementation CLEAN review, in that order | instance profiles, remaining kind consumers, later Projection engine, generic CLI/SDK operations |
| Instance profile and descriptor selection | HCM-1.2 owns closed descriptors, exact sources, bounded layering, deterministic selection, and the resolved-profile fingerprint; HCM-1.3 derives the selected registry; HCM-1.4 makes setup/doctor consume the same closure. HCM-2.1 adopts selected `project_context`; completed HCM-2.2 additively selects and consumes `project_authority` through profile `1.1.0`. | `RealPathAdopted` for exact Project Context and first-party Charter; `BoundaryLanded` for engine ownership and broader setup/doctor adoption | `handbook-engine` profile/artifact kernel | Preserve exact-source/no-ambient resolution and convert remaining instances only in their slices | kind/schema registry, project conditions, vocabulary, Context Resolution, remaining layout/flow/content authority |
| Shipped default artifact set | HCM-1.2 packages the exact HCM-0.6 catalog under `handbook.profile.shipped-root@1.0.0`. Completed HCM-2.2 preserves it and adds the standalone `1.1.0` selection delta for `project_authority`. The package still has no root Work Specification, Decision Record, or Risk Record instance. | `RealPathAdopted` for exact Project Context and Charter content; `BoundaryLanded` for exact package data and bounded setup/doctor decisions; `TargetOnly` for remaining content authority/materialization/condition evaluation | Phase 0 decision authority, then `handbook-engine` shipped profile data | Preserve the literal closure and unique constitutional root; add remaining behavior only in separately reviewed slices | kind registry, project condition evaluator, remaining content authority, skill UX |
...
| Structured authoring | HCM-2.1 makes Project Context canonical YAML the only selected editable truth. Completed HCM-2.2 constructs candidate `1.3` in three modes, independently binds exact result bytes, promotes only new approval, and renders deterministic Markdown. Environment Inventory content remains unconverted. | `ContractCorrectAndProven` for exact Project Context and first-party Charter; `UsefulPrecursor` for unconverted siblings | `handbook-engine` | Preserve both canonical owners and convert remaining families separately | remaining artifact-kind schemas/renderers, later Projections, bridge deletion gate |
| Charter intake coverage | Completed HCM-2.2 implements the versioned 16-coverage `CharterIntakeDefinition`, three equivalent acquisition modes, detailed provenance, result-independent candidate subject, engine-owned result `1.0`, closed final exact-result binding, stable locked replay, and orphan/migration refusal. HCM-2.3 documentation describes the exact two-coverage registry-brief intake, but the current production publication boundary is known non-authoritative and cannot make that lineage landed evidence. | `ContractCorrectAndProven` for the exact first-party Charter; `TargetOnly` for the current exact HCM-2.3 registry-brief custom-kind intake and broader generic/custom-kind intake | `handbook-engine` intake/lineage kernel + focused generic CLI adapter | Preserve exact Charter authority and the current exact registry-brief `TargetOnly` baseline; do not apply the frozen future `TargetOnly -> RealPathAdopted` diff or close the registry-brief proof lineage until the ordered HCM-2.3 documentation review, sixteen-surface approval, selector, atomic implementation, Checkpoint C/full proof, GitNexus, and complete implementation review gates all pass | generic kind selection, posture kernel, later SDK |
...
| Markdown validation | HCM-2.1 validates selected Project Context canonical YAML first and treats Markdown only as a deterministic renderer witness. HCM-2.2 supplies the analogous selected Charter owner and renderer. Fixed siblings remain artifact-specific Markdown. | `ContractCorrectAndProven` for exact Project Context and first-party Charter; `UsefulPrecursor` for fixed siblings | `handbook-engine` | Preserve canonical-first truth and convert siblings separately | remaining canonical YAML, later Projection proof |
