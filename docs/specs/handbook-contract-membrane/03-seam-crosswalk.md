...
| Context Resolution definition stack | HCM-1.2 packages and closure-fingerprints the exact four-level/six-domain shipped stack. The fresh HCM-3.2 candidate executes the ordered stack and all six dimensions, materializes root/child envelopes, enforces monotonic narrowing, deny-on-overlap, indeterminate refusal, typed mutation/memory/validation, append-only escalation and memory-promotion dispositions, the private JCS authority capsule through generic lineage, authenticated publisher/currentness checks, and bounded replacement/quarantine recovery. The complete proof wall, including valid anchored candidate A plus partial scratch refusing different otherwise-valid candidate B before mutation, is GREEN. | `RealPathAdopted` proof candidate; `ContractCorrectAndProven` only after a forthcoming fresh-parent complete-subject implementation/proof review returns `CLEAN` and the parent records it in a completed v1.4 handoff | shared Resolution model with owning execution slices | Obtain and record that fresh-parent `CLEAN` review; preserve exact stack/profile/lineage/currentness authority and keep setup/doctor migration, HCM-3.3+, public transport, and generic framework expansion separately selected | profile selection, flow resolver, pipeline work levels, Snapshot Memory |
