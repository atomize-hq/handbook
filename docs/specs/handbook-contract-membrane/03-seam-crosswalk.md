...
complete first-party Charter implementation and real product-path proof are
...
public author-replay preservation/refusal tests. Adoption requires a different-
fresh `CLEAN` re-review. Fresh implementation Review 3 then proved candidate
...
coherent-forgery variants at all four runtime seams are proof-complete.
Adoption still requires the primary implementation commit and parent
closeout. No HCM-2.3 authority exists.
...
|---|---|---|---|---|---|
| Canonical artifact identities | `CanonicalArtifactKind` remains the fixed pre-membrane Markdown product projection for unmigrated sibling seams. HCM-1.3 owns the selected-profile universe through `ResolvedArtifactRegistry`; HCM-1.4 setup/doctor consume one typed closure. HCM-2.1 makes selected `project_context` descriptor-owned. The HCM-2.2 subject now makes `project_authority` descriptor-owned through candidate `1.3`, exact result authority, approval, promotion, and lifecycle currentness. | `RealPathAdopted` for exact Project Context and first-party Charter; `BoundaryLanded` for the broader selected-profile registry/setup/doctor boundary; `UsefulPrecursor` for unchanged fixed siblings | `handbook-engine` profile/artifact kernel | Preserve both selected owners and remove mixed-family bridges only after separately reviewed cutovers | kind meta-schema, profile schema, generic validation, remaining layout/flow/content authority |
| Artifact kind/schema registry | HCM-1.1 supplies the fail-closed stable-role/schema/kind owner boundary. HCM-1.2 adds six exact package-owned structural schema closures and the constitutional capability. HCM-2.1 consumes Project Context. The HCM-2.2 subject additively admits Project Authority `1.1.0`, its typed supporting definitions, and candidate `1.3` while preserving every released definition plus result `1.0` and intent `1.2`. | `BoundaryLanded` for the exact shipped Charter closure and prior registry/meta-validation boundary; `TargetOnly` for generic intake and remaining kind consumers | `handbook-engine` | Preserve exact refs and byte closure; prove generic custom-kind intake only in HCM-2.3 | instance profiles, remaining kind consumers, later Projection engine, generic CLI/SDK operations |
| Instance profile and descriptor selection | HCM-1.2 owns closed descriptors, exact sources, bounded layering, deterministic selection, and the resolved-profile fingerprint; HCM-1.3 derives the selected registry; HCM-1.4 makes setup/doctor consume the same closure. HCM-2.1 adopts selected `project_context`; the HCM-2.2 subject additively selects and consumes `project_authority` through profile `1.1.0`. | `RealPathAdopted` for exact Project Context and first-party Charter; `BoundaryLanded` for engine ownership and broader setup/doctor adoption | `handbook-engine` profile/artifact kernel | Preserve exact-source/no-ambient resolution and convert remaining instances only in their slices | kind/schema registry, project conditions, vocabulary, Context Resolution, remaining layout/flow/content authority |
| Shipped default artifact set | HCM-1.2 packages the exact HCM-0.6 catalog under `handbook.profile.shipped-root@1.0.0`. The HCM-2.2 subject preserves it and adds the standalone `1.1.0` selection delta for `project_authority`. The package still has no root Work Specification, Decision Record, or Risk Record instance. | `RealPathAdopted` for exact Project Context and Charter content; `BoundaryLanded` for exact package data and bounded setup/doctor decisions; `TargetOnly` for remaining content authority/materialization/condition evaluation | Phase 0 decision authority, then `handbook-engine` shipped profile data | Preserve the literal closure and unique constitutional root; add remaining behavior only in separately reviewed slices | kind registry, project condition evaluator, remaining content authority, skill UX |
