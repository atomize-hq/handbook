...
Only the exact registry-brief subsets named below promote; first-party Charter
stays `ContractCorrectAndProven`, broader generic/custom-kind intake stays
`TargetOnly`, and HCM-2.4 and later work remain unauthorized. The later
...
| Artifact kind/schema registry | HCM-1.1 supplies the fail-closed stable-role/schema/kind owner boundary. HCM-1.2 adds six exact package-owned structural schema closures and the constitutional capability. HCM-2.1 consumes Project Context. Completed HCM-2.2 additively admits Project Authority `1.1.0`, its typed supporting definitions, and candidate `1.3` while preserving every released definition plus result `1.0` and intent `1.2`. HCM-2.3 lands one repository-selected registry-brief kind/schema/intake closure through the generic engine and actual-binary CLI path, with atomic publication, replay, refusal, restart, and concurrency proof. | `RealPathAdopted` for the exact HCM-2.3 registry-brief subset; `TargetOnly` for remaining generic intake/kind consumers; `BoundaryLanded` for the exact shipped Charter closure and prior registry/meta-validation boundary | `handbook-engine` | Preserve the exact registry-brief path and its atomic evidence; keep all other custom kinds, shipped-default expansion, remote schemas, generated commands, Projection, SDK, and public transport outside this promotion | instance profiles, remaining kind consumers, later Projection engine, generic SDK operations |
