...
|---|---|---|---|---|---|
| Canonical artifact identities | HCM-1.3 owns the selected-profile universe through `ResolvedArtifactRegistry`; HCM-1.4 setup/doctor consume one typed closure. HCM-2.1 makes selected `project_context` descriptor-owned. Completed HCM-2.2 makes `project_authority` descriptor-owned through candidate, approval, promotion, and lifecycle currentness. The HCM-2.4 greenfield correction makes selected optional `environment_context` canonical YAML/advisory context, and P3 makes repository-profile-selected `work_specification` canonical YAML the Stage 10 capture/handoff authority. P4's Decision negative-surface/path proof is GREEN and completed; P5 is GREEN and accepted. Preserved P6 implementation commit `b1edf060710bc96cd61631755519baba864ba30a` makes descriptor-selected `CanonicalArtifacts` the aggregate collection but retains residual fixed-kind authority identified by the valid discovery review; P6 acceptance remains pending implementation remediation and fresh complete-subject review. | `BoundaryLanded` for the current aggregate implementation; `RealPathAdopted` remains limited to the earlier exact Project Context, Charter, Environment Context, and Stage 10 Work Specification paths until P6 acceptance | `handbook-engine` profile/artifact kernel | Remove residual fixed-kind authority while preserving the adopted owners, accepted Decision/Risk proof, retained exact bytes/identity, and distinct source/render fingerprints; P7 remains ineligible until P6 acceptance | kind meta-schema, profile schema, generic validation, later Projection proof |
| Artifact kind/schema registry | HCM-1.1 supplies the fail-closed stable-role/schema/kind owner boundary. HCM-1.2 adds six exact package-owned structural schema closures and the constitutional capability. HCM-2.1 consumes Project Context. Completed HCM-2.2 admits Project Authority `1.1.0` and its candidate/promotion chain. HCM-2.3 lands one repository-selected registry-brief kind/schema/intake closure. HCM-2.4 now owns exact Environment Context schema `1.1` plus corrected-in-place kind `1.1`, intake `1.0`, and renderer `1.0` definitions, as well as the Work Specification path; rejected P2 evidence definitions are absent. Decision/Risk exact positive mutation/promotion paths are GREEN, and P4's separate five-part negative-surface/path proof is GREEN and completed. | `RealPathAdopted` for the exact registry-brief, Environment Context, and Work Specification subsets; `BoundaryLanded` for the exact shipped Charter closure; `TargetOnly` for remaining generic consumers | `handbook-engine` | Preserve the admitted Environment Context, Work Specification, Decision, and Risk evidence; keep task-gate runtime, remote schemas, generated commands, Projection, SDK, and public transport outside this packet | instance profiles, remaining kind consumers, later Projection engine, generic SDK operations |
...
| Canonical layout | HCM-2.1 takes `.handbook/project/context.yaml` from the selected Project Context descriptor, and the HCM-2.4 greenfield correction takes `.handbook/project/environment.yaml` from the selected Environment Context descriptor through the trusted safe loader and advisory flow path. Preserved P6 implementation commit `b1edf060710bc96cd61631755519baba864ba30a` removes fixed layout/path selection, while the separate residual fixed-kind authority finding remains open. | `RealPathAdopted` for the exact Project Context and Environment Context paths; `BoundaryLanded` for the unaccepted aggregate layout | `handbook-engine` | Preserve descriptor-owned paths, trusted repository-relative safety, and exact admitted identity while the P6 review gate remains open | artifact registry, trusted repo-relative paths, later Projection proof |
| Structured authoring | HCM-2.1 makes Project Context canonical YAML the only selected editable truth. Completed HCM-2.2 does the same for Charter. HCM-2.4 makes Environment Context canonical YAML through the generic intake/candidate/promotion writer and derives Markdown only as a view; P3 does likewise for Stage 10 Work Specification. Legacy Environment Inventory Markdown has no authority or readiness influence. Decision/Risk exact mutation/promotion paths are GREEN, and P4's separate negative-surface/path proof is GREEN and completed. Preserved P6 implementation commit `b1edf060710bc96cd61631755519baba864ba30a` removes the aggregate bridges but remains unaccepted while residual fixed-kind authority is open. | `ContractCorrectAndProven` for the exact pre-P6 Project Context, Charter, Environment Context, and Stage 10 Work Specification paths; `BoundaryLanded` for their unaccepted aggregate use | `handbook-engine` plus `handbook-pipeline` for the exact Stage 10 path | Preserve the four canonical owners, their accepted Decision/Risk proofs, and renderer-only derived views while completing P6 remediation | remaining artifact-kind behavior, later Projections |
...
| Markdown validation | HCM-2.1 validates selected Project Context canonical YAML first and treats Markdown only as a deterministic renderer witness. HCM-2.2 supplies the analogous selected Charter owner and renderer. The HCM-2.4 greenfield correction validates Environment Context YAML first and uses its deterministic Markdown only as an advisory packet view. P3/P3B validate canonical Work Specification YAML first and retain Feature Spec Markdown only as a deterministic human-review view. Preserved P6 implementation commit `b1edf060710bc96cd61631755519baba864ba30a` removes bridge authority and preserves canonical-first truth, but residual fixed-kind authority keeps aggregate P6 validation unaccepted. | `ContractCorrectAndProven` for exact pre-P6 Project Context, first-party Charter, Environment Context, and Stage 10 Work Specification paths; `BoundaryLanded` for aggregate descriptor-selected validation | `handbook-engine` plus `handbook-flow` advisory consumption and the exact `handbook-pipeline` Stage 10 path | Preserve canonical-first truth and renderer-only Markdown witnesses while closing the P6 review gate | remaining canonical YAML, later Projection proof |
| Setup scaffolding | HCM-1.4 replaces fixed artifact selection with typed selected-profile decisions; HCM-2.1 adds Project Context inspection. HCM-2.2 keeps setup non-authoring and adds strict repository-identity initialization. HCM-2.4 adds optional Environment Context status: valid context is preserved, while missing or invalid unselected context reports non-blockingly. | `RealPathAdopted` for repository identity, profile-aware readiness, Project Context inspection, and Environment Context advisory status; `TargetOnly` for remaining materialization | SDK/use case over engine | Preserve non-authoring setup and add approved materialization only in its own slice | profile loader, approver registry, remaining content authority, CLI UX |
...
| Flow resolver | Preserved P6 implementation commit `b1edf060710bc96cd61631755519baba864ba30a` replaces the HCM-2.1/HCM-2.2 bridges and mixed fixed collection with one descriptor-selected `CanonicalArtifacts` collection, but the valid discovery review found residual fixed-kind authority in normal flow identity and decisions. The pre-edit selector closure is CLEAN only for remediation sufficiency; the implementation remains unaccepted. | `BoundaryLanded` for the unaccepted descriptor-selected aggregate flow | `handbook-flow` | Propagate exact admitted instance ID, kind ref, label, path, bytes, and distinct source/render fingerprints without fixed-kind authority; preserve Charter, Project Context, Environment Context, packet, budget, fixture, blocker, refusal, ordering, and fingerprint behavior | profile kernel, Resolution envelope, SDK DTOs |
