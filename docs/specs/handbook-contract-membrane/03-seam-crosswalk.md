...
|---|---|---|---|---|---|
| Canonical artifact identities | HCM-1.3 owns the selected-profile universe through `ResolvedArtifactRegistry`; HCM-1.4 setup/doctor consume one typed closure. HCM-2.1 makes selected `project_context` descriptor-owned. Completed HCM-2.2 makes `project_authority` descriptor-owned through candidate, approval, promotion, and lifecycle currentness. The HCM-2.4 greenfield correction makes selected optional `environment_context` canonical YAML/advisory context, and P3 makes repository-profile-selected `work_specification` canonical YAML the Stage 10 capture/handoff authority. P4's Decision negative-surface/path proof is GREEN and completed; P5 is GREEN and accepted. The complete P6 repair candidate makes descriptor-selected `CanonicalArtifacts` the aggregate collection and removes residual normal-path fixed-kind authority while retaining exact admitted instance ID, kind ref, label, path, bytes, and distinct source/render fingerprints. | `ContractCorrectAndProven` for the aggregate implementation only when the frozen final-review supplemental-2 closure dispatch is CLEAN; otherwise `BoundaryLanded`; `RealPathAdopted` remains limited to the earlier exact Project Context, Charter, Environment Context, and Stage 10 Work Specification paths | `handbook-engine` profile/artifact kernel | Bind P6 acceptance only to CLEAN in `handoffs/dispatches/20260731T004500Z--HCM-2-4--p6-final-review-control-truth-supplemental-2-closure.json`; preserve adopted owners and accepted Decision/Risk proof; P7 remains unselected even if eligible | kind meta-schema, profile schema, generic validation, later Projection proof |
...
| Canonical layout | HCM-2.1 takes `.handbook/project/context.yaml` from the selected Project Context descriptor, and the HCM-2.4 greenfield correction takes `.handbook/project/environment.yaml` from the selected Environment Context descriptor through the trusted safe loader and advisory flow path. The complete P6 repair candidate removes fixed layout/path and normal-path fixed-kind selection. | `RealPathAdopted` for the exact Project Context and Environment Context paths; aggregate layout becomes `ContractCorrectAndProven` only under the frozen CLEAN condition | `handbook-engine` | Preserve descriptor-owned paths, trusted repository-relative safety, and exact admitted identity | artifact registry, trusted repo-relative paths, later Projection proof |
| Structured authoring | HCM-2.1 makes Project Context canonical YAML the only selected editable truth. Completed HCM-2.2 does the same for Charter. HCM-2.4 makes Environment Context canonical YAML through the generic intake/candidate/promotion writer and derives Markdown only as a view; P3 does likewise for Stage 10 Work Specification. Legacy Environment Inventory Markdown has no authority or readiness influence. Decision/Risk exact mutation/promotion paths are GREEN, and P4's separate negative-surface/path proof is GREEN and completed. The complete P6 repair candidate removes aggregate bridge and fixed-kind authority without changing those owners. | `ContractCorrectAndProven` for the exact Project Context, Charter, Environment Context, and Stage 10 Work Specification paths; aggregate use inherits that classification only under the frozen CLEAN condition | `handbook-engine` plus `handbook-pipeline` for the exact Stage 10 path | Preserve the four canonical owners, their accepted Decision/Risk proofs, and renderer-only derived views | remaining artifact-kind behavior, later Projections |
...
| Markdown validation | HCM-2.1 validates selected Project Context canonical YAML first and treats Markdown only as a deterministic renderer witness. HCM-2.2 supplies the analogous selected Charter owner and renderer. The HCM-2.4 greenfield correction validates Environment Context YAML first and uses its deterministic Markdown only as an advisory packet view. P3/P3B validate canonical Work Specification YAML first and retain Feature Spec Markdown only as a deterministic human-review view. The complete P6 repair candidate preserves canonical-first truth and removes bridge/fixed-kind validation authority. | `ContractCorrectAndProven` for exact Project Context, first-party Charter, Environment Context, and Stage 10 Work Specification paths; aggregate descriptor-selected validation receives that classification only under the frozen CLEAN condition | `handbook-engine` plus `handbook-flow` advisory consumption and the exact `handbook-pipeline` Stage 10 path | Preserve canonical-first truth and renderer-only Markdown witnesses | remaining canonical YAML, later Projection proof |
...
| Flow resolver | The complete P6 repair candidate replaces the HCM-2.1/HCM-2.2 bridges and mixed fixed collection with one descriptor-selected `CanonicalArtifacts` collection; propagates exact admitted instance ID, kind ref, label, path, bytes, and distinct source/render fingerprints; and removes normal-path fixed-kind authority. The complete packet/workspace proof wall is GREEN. | `ContractCorrectAndProven` only if the frozen final-review dispatch is CLEAN; otherwise `BoundaryLanded` | `handbook-flow` | Preserve Charter, Project Context, Environment Context, packet, budget, fixture, blocker, refusal, ordering, and fingerprint behavior; keep P7 unselected | profile kernel, Resolution envelope, SDK DTOs |
