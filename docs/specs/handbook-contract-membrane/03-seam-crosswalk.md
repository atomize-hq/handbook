...
|---|---|---|---|---|---|
| Canonical artifact identities | `CanonicalArtifactKind` remains the fixed pre-membrane product projection for unmigrated sibling seams and aggregate flow. HCM-1.3 owns the selected-profile universe through `ResolvedArtifactRegistry`; HCM-1.4 setup/doctor consume one typed closure. HCM-2.1 makes selected `project_context` descriptor-owned. Completed HCM-2.2 makes `project_authority` descriptor-owned through candidate `1.3`, exact result authority, approval, promotion, and lifecycle currentness. HCM-2.4 P1A/P1C add exact successor definitions and select shipped-root `1.2`; P3 makes repository-profile-selected `work_specification` canonical YAML the Stage 10 capture/handoff authority. | `RealPathAdopted` for exact Project Context, first-party Charter, and the exact Stage 10 Work Specification path; `BoundaryLanded` for the broader selected-profile registry and exact HCM-2.4 successor definitions; `UsefulPrecursor` for unchanged fixed siblings and aggregate flow | `handbook-engine` profile/artifact kernel | Preserve the three adopted owners; finish Environment Context and Decision/Risk positive paths before removing mixed-family bridges in P6 | kind meta-schema, profile schema, generic validation, remaining layout/flow/content authority |
| Artifact kind/schema registry | HCM-1.1 supplies the fail-closed stable-role/schema/kind owner boundary. HCM-1.2 adds six exact package-owned structural schema closures and the constitutional capability. HCM-2.1 consumes Project Context. Completed HCM-2.2 additively admits Project Authority `1.1.0`, its typed supporting definitions, and candidate `1.3` while preserving every released definition plus result `1.0` and intent `1.2`. HCM-2.3 lands one repository-selected registry-brief kind/schema/intake closure through the generic engine and actual-binary CLI path. HCM-2.4 P1A package-admits exact successor kind/intake/fixed-renderer closures for Project Context, Environment Context, Work Specification, Decision Record, and Risk Record without changing released schemas; P3 proves the exact Work Specification real path, while Decision/Risk positive mutation remains unproven. | `RealPathAdopted` for the exact HCM-2.3 registry-brief and HCM-2.4 Work Specification subsets; `BoundaryLanded` for the five exact HCM-2.4 successor definition closures and the exact shipped Charter closure; `TargetOnly` for remaining generic intake/kind consumers and unproven Decision/Risk positive mutation | `handbook-engine` | Preserve the exact registry-brief and Work Specification evidence; do not promote Environment Context or Decision/Risk behavior until their separate authority gates close; keep remote schemas, generated commands, Projection, SDK, and public transport outside this promotion | instance profiles, remaining kind consumers, condition evaluator, later Projection engine, generic SDK operations |
| Instance profile and descriptor selection | HCM-1.2 owns closed descriptors, exact sources, bounded layering, deterministic selection, and the resolved-profile fingerprint; HCM-1.3 derives the selected registry; HCM-1.4 makes setup/doctor consume the same closure. HCM-2.1 adopts selected `project_context`; completed HCM-2.2 selects and consumes `project_authority` through profile `1.1.0`. HCM-2.4 P1C selects shipped-root `1.2` with exactly Project Context, Project Authority, and Environment Context, while preserving the HCM-2.2 Charter identity pair. P3 separately proves a repository-profile-selected Work Specification descriptor. | `RealPathAdopted` for exact Project Context, first-party Charter, and the exact Work Specification Stage 10 path; `BoundaryLanded` for shipped-root `1.2` selection and broader setup/doctor adoption; `TargetOnly` for Environment Context behavior and Decision/Risk positive mutation | `handbook-engine` profile/artifact kernel | Preserve exact-source/no-ambient resolution and the three-instance shipped root; convert blocked instances only after their explicit authority prerequisites | kind/schema registry, project conditions, vocabulary, Context Resolution, remaining layout/flow/content authority |
| Shipped default artifact set | HCM-2.4 P1C selects exact `handbook.profile.shipped-root@1.2.0` with only Project Context, Project Authority, and conditional Environment Context. Released Charter authority retains its HCM-2.2 `1.1` identity pair. The package still has no root Work Specification, Decision Record, or Risk Record instance, and Environment Context behavior remains unconverted pending condition-evaluator authority. | `RealPathAdopted` for exact Project Context and Charter content; `BoundaryLanded` for exact shipped-root `1.2` package data and selected Environment Context metadata; `TargetOnly` for Environment Context materialization/condition evaluation and root Work/Decision/Risk instances | Phase 0 decision authority, then `handbook-engine` shipped profile data | Preserve the literal three-instance closure and unique constitutional root; add remaining behavior only through the blocked packet selectors | kind registry, project condition evaluator, remaining content authority, skill UX |
...
| Structured authoring | HCM-2.1 makes Project Context canonical YAML the only selected editable truth. Completed HCM-2.2 constructs candidate `1.3` in three modes, independently binds exact result bytes, promotes only new approval, and renders deterministic Markdown. HCM-2.4 P3 makes Stage 10 Work Specification YAML canonical and derives Feature Spec Markdown only as a view. Environment Inventory content remains unconverted; Decision/Risk have retained definition/fixture/read/validate evidence but no authorized positive mutation. | `ContractCorrectAndProven` for exact Project Context, first-party Charter, and the exact Stage 10 Work Specification path; `UsefulPrecursor` for unconverted or positive-path-incomplete siblings | `handbook-engine` plus `handbook-pipeline` for the exact Stage 10 path | Preserve all three canonical owners; resume blocked families only under their exact authority selectors before bridge deletion | remaining artifact-kind behavior, condition evaluator, later Projections, bridge deletion gate |
...
| Markdown validation | HCM-2.1 validates selected Project Context canonical YAML first and treats Markdown only as a deterministic renderer witness. HCM-2.2 supplies the analogous selected Charter owner and renderer. HCM-2.4 P3/P3B validate canonical Work Specification YAML first and retain Feature Spec Markdown only as a deterministic human-review view; blocked siblings and aggregate flow remain unchanged. | `ContractCorrectAndProven` for exact Project Context, first-party Charter, and the exact Stage 10 Work Specification path; `UsefulPrecursor` for fixed or blocked siblings | `handbook-engine` plus exact `handbook-pipeline` Stage 10 path | Preserve canonical-first truth; complete blocked siblings and aggregate bridge deletion separately | remaining canonical YAML, condition evaluator, later Projection proof |
| Setup scaffolding | HCM-1.4 replaces fixed artifact selection with typed selected-profile decisions; HCM-2.1 adds Project Context inspection. HCM-2.2 keeps setup non-authoring and adds strict repository-identity initialization. | `RealPathAdopted` for repository identity, profile-aware readiness, and Project Context inspection; `TargetOnly` for remaining materialization | SDK/use case over engine | Preserve non-authoring setup and add approved materialization only in its own slice | profile loader, approver registry, remaining content authority, CLI UX |
| Doctor baseline | HCM-1.4 exposes the typed profile/readiness report; HCM-2.1 advances it to `1.1.0` with Project Context fingerprints. HCM-2.2 advances it to `1.2.0` with repository-identity and retained Charter observation rows. | `RealPathAdopted` for exact Project Context and Charter rows; `BoundaryLanded` for broader profile-aware machine truth | SDK + CLI renderer | Carry typed reporting forward without widening authority | artifact registry, condition evaluator, JSON protocol |
| Flow resolver | HCM-2.1 installs `BR-HCM-2-PILOT-FLOW-01` for selected Project Context. HCM-2.2 installs `BR-HCM-2-CHARTER-FLOW-01`, C04 `reduced-v1-m8.3`, and distinct retained Charter source/render fingerprints. | `RealPathAdopted` for exact Project Context and Charter mixed-family flow; `BoundaryLanded` for the reduced scope | `handbook-flow` | Preserve source/render budgets and remove temporary bridges by their reviewed deletion gate | profile kernel, remaining family cutovers, Resolution envelope, SDK DTOs |
