...
|---|---|---|---|---|---|
| Canonical artifact identities | `CanonicalArtifactKind` remains the fixed pre-membrane product projection for unmigrated sibling seams and aggregate flow. HCM-1.3 owns the selected-profile universe through `ResolvedArtifactRegistry`; HCM-1.4 setup/doctor consume one typed closure. HCM-2.1 makes selected `project_context` descriptor-owned. Completed HCM-2.2 makes `project_authority` descriptor-owned through candidate, approval, promotion, and lifecycle currentness. The HCM-2.4 greenfield correction makes selected optional `environment_context` canonical YAML/advisory context, and P3 makes repository-profile-selected `work_specification` canonical YAML the Stage 10 capture/handoff authority. P4's Decision negative-surface/path proof is GREEN and completed; P5 is GREEN and accepted. P6 is selected for a later implementation session but unstarted. | `RealPathAdopted` for exact Project Context, Charter, Environment Context, and Stage 10 Work Specification paths; `BoundaryLanded` for the broader selected-profile registry; `UsefulPrecursor` for unchanged fixed siblings and aggregate flow | `handbook-engine` profile/artifact kernel | Preserve the adopted owners and accepted Decision/Risk proof; execute only the reviewed P6 selector before removing mixed-family bridges | kind meta-schema, profile schema, generic validation, remaining layout/flow/content authority |
...
| Canonical layout | `CanonicalLayoutContract` still carries fixed-family fields. HCM-2.1 takes `.handbook/project/context.yaml` from the selected Project Context descriptor, and the HCM-2.4 greenfield correction takes `.handbook/project/environment.yaml` from the selected Environment Context descriptor through the trusted safe loader and advisory flow path. P6 is selected but unimplemented. | `RealPathAdopted` for the exact Project Context and Environment Context paths; `UsefulPrecursor` for the broader fixed layout | `handbook-engine` | Preserve both descriptor-owned paths; remove remaining dedicated fixed-family layout selection and both temporary bridges only through the reviewed P6 selector | artifact registry, trusted repo-relative paths, aggregate flow cleanup |
| Structured authoring | HCM-2.1 makes Project Context canonical YAML the only selected editable truth. Completed HCM-2.2 does the same for Charter. HCM-2.4 makes Environment Context canonical YAML through the generic intake/candidate/promotion writer and derives Markdown only as a view; P3 does likewise for Stage 10 Work Specification. Legacy Environment Inventory Markdown has no authority or readiness influence. Decision/Risk exact mutation/promotion paths are GREEN, and P4's separate negative-surface/path proof is GREEN and completed. | `ContractCorrectAndProven` for exact Project Context, Charter, Environment Context, and Stage 10 Work Specification paths; `UsefulPrecursor` for remaining sibling conversion and aggregate flow | `handbook-engine` plus `handbook-pipeline` for the exact Stage 10 path | Preserve the four canonical owners and their accepted Decision/Risk proofs; P6 is selected only for the reviewed aggregate bridge deletion | remaining artifact-kind behavior, later Projections, aggregate bridge deletion gate |
...
| Flow resolver | HCM-2.1 installs `BR-HCM-2-PILOT-FLOW-01` for selected Project Context. HCM-2.2 installs `BR-HCM-2-CHARTER-FLOW-01`, C04 `reduced-v1-m8.3`, and distinct retained Charter source/render fingerprints. HCM-2.4 adds selected valid Environment Context to the advisory packet as a deterministic rendered source; missing or invalid context is omitted and reported without blocking, and file presence never selects enforcement. P6 is selected and entry-authorized but unimplemented. | `RealPathAdopted` for exact Project Context, Charter, and Environment Context mixed-family flow; `BoundaryLanded` for the reduced scope | `handbook-flow` | Preserve advisory omission/reporting and source/render budgets; remove temporary bridges only in the later session under the reviewed P6 deletion selector | profile kernel, remaining family cutovers, Resolution envelope, SDK DTOs |
