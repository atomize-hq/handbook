...
|---|---|---|---|---|---|---|---|---|---|---|---|
| `HCM-RF-0001` | `P3` | `accepted` | `handbook-pipeline \| pipeline handoff view-independence proof \| HCM-2.4 P3 view non-authority gate \| feature-id and bundle-root cross-case assertions absent` | The Markdown-view mutation/deletion test does not explicitly compare `manifest.feature_id` or emitted bundle root across its two cases. | HCM-2.4; `20260727T014349Z--HCM-2-4--p3-work-specification-closure-review`; `/root/hcm_2_4_p3_closure_review` | `crates/pipeline/tests/pipeline_handoff.rs:406`; `docs/specs/handbook-contract-membrane/slices/HCM-2.4/proof/implementation/P3-work-specification.md`; `docs/specs/handbook-contract-membrane/handoffs/dispatches/20260727T014349Z--HCM-2-4--p3-work-specification-closure-review.json`; `docs/specs/handbook-contract-membrane/slices/HCM-2.4/proof/implementation/P7-phase-2-exit-closeout.md` | Test-only assertions in `handoff_ignores_persisted_markdown_view_mutation_and_deletion`; no runtime surface | Phase-exit triage accepts this as non-blocking defense-in-depth debt: canonical-byte/input-SHA equality, view exclusion, validated-manifest equality, stale-provenance refusal, and the P7 real handoff wall prove the selected gate. No P7 runtime or test edit is authorized. | unassigned test-only maintenance | `20260727T064246Z--HCM-2-4--partial-control-pack-complete-subject-review`; `/root/hcm_2_4_partial_closeout_review` | none |
| `HCM-RF-0002` | `P3` | `accepted` | `handbook-cli \| Unix route-state refusal exact-message test \| cross-platform CLI regression wall \| Windows-specific expected wording on an absolute Unix path` | The full WSL `cli_surface` target rejects `/tmp/CHARTER.md` correctly but one exact-string assertion expects the Windows component-validation wording instead of the Unix absolute-path wording. | HCM-2.4 P7 parent proof replay | `crates/cli/tests/cli_surface.rs:4282`; `crates/cli/tests/cli_surface.rs:4346`; `crates/pipeline/src/route_state.rs:1873`; `crates/compiler/tests/pipeline_state_store.rs:1183`; `crates/pipeline/tests/pipeline_state_store.rs:1279`; `docs/specs/handbook-contract-membrane/slices/HCM-2.4/proof/implementation/P7-phase-2-exit-closeout.md` | One cross-platform CLI exact-message assertion; runtime refusal and mutation safety are unchanged | Fresh WSL replay returned 100/101 and reproduced only this wording mismatch. Both platforms refuse before mutation, lower compiler/pipeline Unix negatives pass, every P1C Unix-selected-profile target passes, and no Phase 2 exit row depends on the message variant. P7 cannot edit tests. | unassigned cross-platform test maintenance | none | none |
| `HCM-RF-0003` | `P4` | `accepted` | `HCM-4.2 proof strategy \| definition-pin refusal labels \| five-way admission distinction \| duplicated missing label obscures registry-missing` | The proof strategy says “missing/malformed/missing/mismatched/stale definition pin,” duplicating `missing` instead of naming the distinct registry-missing case. | HCM-4.2; `20260811T034900Z--HCM-4-2--causal-remediation-complete-subject-final-review`; `/root/hcm42_final_review` | `docs/specs/handbook-contract-membrane/slices/HCM-4.2/proof/strategy.md:18`; `docs/specs/handbook-contract-membrane/05-contracts-schemas-and-gates.md:4438`; `docs/specs/handbook-contract-membrane/slices/HCM-4.2/review/20260811T035900Z--causal-remediation-complete-subject-final-review.md` | One non-normative HCM-4.2 proof-strategy label; the normative five-way 05 contract and selected proof remain correct | Retained after the fresh complete-subject CLEAN review. Rewriting the frozen subject or running a post-CLEAN cycle for wording polish is prohibited; no runtime behavior, authority, admission outcome, or required proof is missing. | unassigned documentation maintenance under future fresh authority | none | none |
