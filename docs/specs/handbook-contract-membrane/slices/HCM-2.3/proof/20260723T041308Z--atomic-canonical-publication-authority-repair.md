# HCM-2.3 atomic canonical-publication authority and Review 1/2/3/4/5/6/7/8/9 remediation
...
- Role: documentation-only contract-authority repair
...
- Windows retains every ancestor without `FILE_SHARE_DELETE`, enumerates each
  ...
  `SetFileInformationByHandle(FileRenameInfo)` on the DELETE-capable retained
...
and exact 64-byte `FILE_OBJECTID_BUFFER` output. `STATUS_PENDING` refuses.
Absence requires both raw and final `IO_STATUS_BLOCK.Status` to equal
`STATUS_OBJECTID_NOT_FOUND` and `Information == 0`. Dual success with
`Information == 64` means present and refuses; every other or inconsistent
status/length fails closed. `DeviceIoControl`/`GetLastError` is not raw-NTSTATUS
authority and no fallback exists. Present Object ID, unsupported query, access
denial, mutation, zero or all-ones file ID, or any other result fails closed.
...
- Microsoft [`FILE_RENAME_INFO`](https://learn.microsoft.com/en-us/windows/win32/api/winbase/ns-winbase-file_rename_info), updated 2026-05-15, expressly permits a non-null `RootDirectory` for a relative `FileName`; stale excerpts saying it must be null are not current authority
...
64-byte output, and raw-plus-final status policy, and executes every one of its
33 field/status/length mutations. The remaining semantic negatives include arbitrary opaque
...
```powershell
$proof = Get-Content -Raw docs/specs/handbook-contract-membrane/slices/HCM-2.3/proof/20260723T041308Z--atomic-canonical-publication-authority-repair.md
...
need(len(checker["required_ntfscontrolfile_rejection_ids"]) == 33, "required NtFsControlFile rejection count changed")
need(set_hash(checker["required_ntfscontrolfile_rejection_ids"]) == "f5efedf2b4af2e88d6920e858af4d40b63e68cbb61cbcf50ef156ea4fec7aa3f", "required NtFsControlFile rejection IDs changed")
need(digest(matrix) == "a5827529780955e3aafeb18261507abd296fe159e44eb42453405c738383a548", "exact matrix content changed")
...
need(digest(ntfs_contract) == "f4e967c0453ccd1f7dae0f842062c225b477ce8d36847ca18610f06a2334101e", "exact NtFsControlFile contract changed")
...
EXPECTED_NTFSCONTROL_STATUS = {
    "status_pending": {
        ...
        "disposition": "unsupported_native_observation_no_wait_or_retry",
    },
    "object_id_absent": {
...
for row_id, row in rows.items():
    ...
    if any(token in row["action"] for token in ("publish_", "claim_", "restore_")):
        need("SetFileInformationByHandle(FileRenameInfo)" in windows_primitive,
        ...
        need("RootDirectory_retained_parent" in windows_primitive and
             ...
             f"{row_id}/windows: incomplete FileRenameInfo binding")
...
- executable exact matrix/chain checker: PASS for 81 rows, 162 explicit
  ...
  every Windows move uses exact retained-handle `FileRenameInfo`, every
