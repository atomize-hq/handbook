...
User-selected `HCM-2.2-ESC-003` repaired the documentation/control-pack
authority by selecting candidate `1.3` as the independent downstream exact-
result anchor. The later exact-result implementation selector authorized the
complete bounded implementation from clean baseline `f676ea59`; it is now
proof-complete. Fresh implementation Review 1 returned three Required findings;
complete frozen result-schema enforcement, exact approval-quorum replay, and
exact resolved-definition closure are repaired test-first and awaiting a
different-fresh exact-subject re-review. Fresh implementation Review 2 then
required version-exact historical candidate admission; distinct closed
`1.0`/`1.1`/`1.2` validation, internal-ID equality, and public replay
preservation/refusal tests now repair it. Fresh implementation Review 3 then
required exact candidate provenance reconstruction; the central retained-
intake/normalized-content populated-leaf bijection and eight coherent-resigning
author/approval/promotion/recovery matrices now repair `R3-001`. The rejected
implementation is preserved untouched at
`486458acfe8373a977e595a4854ac786166e3e76` and remains non-authoritative
evidence. Do not adopt or commit the implementation, or start HCM-2.3, until
this complete repaired subject receives fresh `CLEAN` review; then use the
prescribed primary and parent-closeout commits.
...
- [x] Revalidate the selected HCM-2.1 closeout, planning closeout, clean implementation entry, and packet fingerprint.
- [x] Reread the required skill chain and named `00`-`08`/HCM-0.6 sections.
- [x] Prove setup create-news and preserves `.handbook/repository-identity.v1`, registry equality is fail-closed, reset-state cannot select it, and every approve/admin operation ID comes from the engine-owned frozen derivation with no caller/adapter identity input.
- [x] Prove the exact typed operation-to-allocator-token table, direct-engine opaque-ID compatibility, both pre-invocation refusal branches and crossed-nullability negatives, the distinct closed unsafe-recovery refusal envelope, predecessor-ordered recovery before identity resolution, no new operation-owned delta beyond separately attributable recovery-table effects, and request/challenge/raw-intent/result/output equality without adding operation IDs to semantic authority records.
- [x] Refresh GitNexus and run upstream impact before every existing-symbol edit.
- [x] Refresh impact and warn before `evaluate_charter_intake` (known HIGH),
...
- [x] Capture the exact old definition/package/install manifest and staged allowed paths.

## RED proof

- [x] Reproduce every literal schema document, normalized-JCS line, length, expected definition/profile fingerprint, old-byte closure, exact dependency ref/current-producer equality, acyclic graph, descriptor-only intake selection, package, and selected-profile vector before implementation.
- [x] Add complete live-field crosswalk and canonical Charter schema/typed/capability positive and exhaustive negative tests.
- [x] Add all-mode equivalence and sixteen-item non-overlapping coverage/source/bijection/unknown/contradiction/waiver RED tests.
- [x] Add additive intake/approval/promotion `1.1`, candidate `1.3`, and lifecycle-validation-result `1.0` basis lineage plus exact result-document binding/replay/orphan closure and exact ES256/`fmt: none` authenticator bootstrap/registry/UV/UP/assertion/class/revocation, distinct add-enrollment credential and P-256 keypair, real registry-resolved DER signature verification, exact `AUTHENTICATOR_UNAVAILABLE` fail-before-delta result, caller-intent-only request DTOs, operation-by-status/refusal result DTOs and mandatory crossed-state rejections, forged mapping, stale authority, replay, and ABA RED tests; mechanically prove every current implementation task names candidate `1.3` and treats candidate `1.0`/`1.1`/`1.2` as historical-only.
- [x] Add a retained lifecycle-head positive with at least two observations whose event-precedence order differs from ref order, plus ref-sorted/reordered, duplicate, incomplete, excess, and retained-head rewrite negatives.
- [x] Add create-null versus mandatory-amendment-basis equality at every lineage
...
- [x] Add literal canonical YAML/Markdown fixtures, independent emitter/renderer, and fingerprint RED goldens.
- [x] Add lifecycle review/reassessment total-transition/precedence/fingerprint/targeted-reopen/no-auto-regeneration RED tests.
- [x] Add compiler/CLI/skill/setup/doctor/Environment Inventory/flow/legacy-irrelevance RED tests.
- [x] Add native non-Unix read-only and pre-mutation-refusal RED tests.

## Versioned definitions and canonical owner

- [x] Add typed closed definition registries/meta-schema validation for intake, renderer, lifecycle, approval, waiver, triggers, and semantic validators.
- [x] Add the exact HCM-2.2 definition refs and packet-published uniform fingerprints; refuse any reproduced-byte mismatch.
- [x] Add Project Authority content schema/kind and compatible constitutional semantic validator `1.1.0` while retaining capability-required validator `1.0.0`, old bytes, and kind/intake acyclicity.
- [x] Add standalone shipped profile `1.1.0` with only the exact Charter selection delta.
- [x] Extend selected-profile resolution additively and replay setup/doctor/author/flow/preflight/CLI immediately.
- [x] Land typed `CanonicalCharter`, complete retained-field crosswalk, duplicate-safe selected-schema parse, semantic-capability validation, and closed canonical emitter.
- [x] Land retained no-follow observation plus exact source fingerprint.
- [x] Land fixed in-memory Markdown renderer plus exact rendered fingerprint.

## Intake, candidate, approval, promotion, lifecycle

- [x] Resolve intake by selected instance/kind exact refs, never filename/mode/prompt.
- [x] Evaluate all sixteen coverage IDs with exact authority/source, non-overlap, leaf-source bijection, and value-versus-minimum-specificity-waiver rules.
- [x] Preserve declarations, evidence, defaults, unknowns, contradictions, waivers, prompt events, and mode as typed provenance.
- [x] Prove guided-adaptive, express, and agent-assisted produce the same candidate schema and equivalent bytes for identical inputs.
- [x] Append additive `1.1` immutable intake records, then construct candidate
...
- [x] Implement exact recovery-before-intent lock/journal/fsync/create-new-or-
  ...
  equality.
- [x] Prove output scratch path/type/random-name/create-new/no-follow/same-
...
- [x] Prove pre-commit recovery requires the exact bound prior lifecycle state,
...
- [x] Implement exact lifecycle observation/transition schemas, review trigger, reassessment mappings, precedence, state fingerprints, standalone event journal/recovery, the global promotion-then-registry-then-lifecycle order with reverse-order refusal and deadlock/concurrency proof, and total transition table.
- [x] Require new approved lineage to clear a lifecycle state; never mutate or auto-regenerate truth.
- [x] Keep all fault-injection traits, constructors, fail points, and controls
...
- [x] Rewire compiler/CLI to exact author/approve/promote/validate grammar and typed results.
- [x] Cut core/generated/installed skills to skill-directed agent acquisition with no nested CLI wizard/model.
- [x] Keep setup non-authoring and extend doctor additively from one retained observation.
- [x] Remove selected old Charter input/Markdown authoring paths after caller cutover with no alias/mapper/importer.
- [x] Cut Environment Inventory's Charter reference only to selected canonical YAML.
- [x] Install `BR-HCM-2-CHARTER-FLOW-01` with selected Charter source/rendered fingerprints.
- [x] Remove legacy Charter influence from all flow/budget/manifest/freshness/log/fixture/carrier paths.
- [x] Advance C04 flow/compiler together to `reduced-v1-m8.3` and preserve C03 only with exact preimage proof.
- [x] Preserve Project Context, Environment Inventory content, Feature Spec, and frozen pipeline behavior outside enumerated carriers.
- [x] Delete obsolete fixed Charter constants/validators/templates only when no selected caller remains.

## Proof and closeout

- [x] Run focused structural, semantic, intake, provenance, lineage, authority, atomicity, renderer, lifecycle, and security tests.
- [x] Run real author/approve/promote/validate, installed skill, setup/doctor, Environment Inventory, and all-three flow smoke.
- [x] Run native Windows proof and every HCM-1/HCM-2.1/workspace/fmt/Clippy/docs regression.
- [x] Replay definition/package/archive/install trees by path, size, SHA-256, and bytes.
- [x] Update only affected `00`, `03`, `04`, `06`, bridge, task, and proof rows.
- [x] Run all handoff, link, archive, scope, secret, whitespace, and diff gates.
- [x] Assemble the exact 41-path Review 1 subject and immutable dispatch; accept
  all three Required findings without waiver.
- [x] Add RED/GREEN coverage and repair frozen result-schema enforcement,
  exact approval-quorum replay, and exact resolved-definition closure; replay
  the complete implementation, product, package, and governance proof wall.
- [x] Assemble the Review 1-remediated exact subject and immutable dispatch.
- [x] Obtain fresh isolated Review 2; accept `HCM-2.2-R2-001` without waiver.
- [x] Reproduce valid-`1.0` false refusal, wrong-internal-ID false admission,
  and additive-`1.1`-field false admission RED; install distinct closed
  historical validators and complete malformed-semantic replay coverage; replay
  the complete implementation, package, product, and governance proof wall.
- [x] Assemble the Review 2-remediated exact subject and immutable dispatch.
- [x] Obtain fresh isolated Review 3; accept `R3-001` without waiver.
- [x] Preserve the eight author/approval/promotion coherent-resigning variants
  RED; add the eight-case pending-recovery preservation matrix; install one
  exact retained-intake/normalized-content populated-leaf bijection gate;
  regenerate both normative chains; replay the complete proof wall; and record
  the additive Review 3 remediation proof.
- [ ] Assemble the Review 3-remediated exact subject and immutable Review 4
  dispatch; obtain a different fresh isolated review and continue until CLEAN.
- [ ] After final `CLEAN`, run GitNexus change detection and confirm only the
  authorized symbols, processes, tests, and authority records changed.
...
- [x] Do not edit released exact-definition bytes or silently fall back to `1.0.0`.
- [x] Do not import/map/migrate/dual-read old Charter input or Markdown.
- [x] Do not persist or edit renderer-derived Markdown.
- [x] Do not let an agent, inference, default, waiver, validator, or setup self-approve/promote constitutional truth.
- [x] Do not build a nested CLI questionnaire or call an LLM inside Handbook.
- [x] Do not implement generic custom-kind intake/rendering, arbitrary executable hooks, or HCM-2.3.
- [x] Do not implement Phase-3 Resolution/Projection/Snapshot/posture-kernel work.
- [x] Do not widen into remaining artifact families, pipeline redesign, SDK/Tauri/Substrate/dock/publication, or unrelated cleanup.
