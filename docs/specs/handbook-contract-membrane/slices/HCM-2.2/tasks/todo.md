...
- [ ] Append additive `1.1` immutable intake records and content-addressed candidates with complete field-source mappings and frozen create/amend basis.
...
- [ ] Re-resolve every definition/profile/candidate/approval/target precondition at promotion.
- [ ] Implement exact recovery-before-intent lock/journal/fsync/create-new-or-equal protocols, exact 72-byte raw-intent-hash commit markers, all 17 fixture-bound content-addressed ref-basename/target-ID/declared-fingerprint/exact-byte equalities, all 17 external exact-ref/current-producer equalities, and total observed-state tables for registry mutations and canonical/promotion/lifecycle records; orphan final content-addressed records rather than deleting them, with end-to-end basis equality, retained registry-pair equality through approval/promotion, ABA, crash, concurrent-reader, and replay proof.
