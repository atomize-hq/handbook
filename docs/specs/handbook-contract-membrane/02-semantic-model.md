...
The HCM-4.1 implementation at checkpoint
`28fa4ab4f2fced48532d6830a7a69d8bd12a0cd1` establishes the SDK/CLI/compiler-
retirement boundary without changing this frozen semantic model. It neither
closes Phase 3 nor proves DTO/schema/bootstrap or transport parity. HCM-4.2
shared DTO/JSON-Schema/bootstrap discovery, HCM-4.3 CLI JSON parity, and
HCM-4.4 Tauri remain separately selected.
...
The approved first-party catalog has exactly six semantic kinds: durable normative project authority, maintained factual project context, maintained environment/runtime facts, bounded intended work/change, discrete project/work decisions, and maintained risk/uncertainty records. The shipped root profile selects exactly `project_authority`, `project_context`, and optional `environment_context`; it selects no work, decision, or risk instance.

Selection is not requiredness, materialization, role, capability, or authority. `project_authority` and `project_context` are always required. `environment_context` is optional advisory context and has no applicability condition. Its presence never selects an execution gate. The unique constitutional root still arises only from the separate `constitutional_root` selection and exact `handbook.capabilities.constitutional-root@1.0.0` conformance.

Environment responsibility is layered: Environment Context names environments and capabilities; a feature/specification owns supported targets, non-negotiable policy, and cross-target completion proof; an implementation task declares portability or exact requirements and receives execution authorization only from an explicitly selected positive session-proof gate. Sprint/orchestration aggregation is reporting and scheduling only. The durable future acceptance gate is frozen in the active HCM-2.4 greenfield decision.
