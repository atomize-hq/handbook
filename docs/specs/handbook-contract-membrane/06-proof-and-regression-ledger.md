...
**Current evidence:** HCM-2.1 makes the closed Project Context `1.0` YAML record selected canonical truth with schema-bound parsing and typed decode; HCM-2.2 does the same for Charter; and the HCM-2.4 greenfield correction makes Environment Context `1.1` canonical YAML with typed decode, bounded load, safe generic mutation, and deterministic derived Markdown.

**Classification:** `ContractCorrectAndProven` for the exact Project Context, Charter, and Environment Context canonical-record paths; `UsefulPrecursor` for retained sibling input models.
...
**Current evidence:** the HCM-2.2 proof-complete implementation subject contains
a versioned 16-coverage intake definition, three acquisition modes, detailed
provenance, candidate/
approval/promotion code, and a deterministic renderer. Option 1 repaired the
candidate/result semantic identity cycle; `HCM-2.2-ESC-002` repaired partial
pending output publication. The later implementation reached fresh Review 3,
which returned `HCM-2.2-AR3-001`: semantic-equal result/witness/mutable-binding
audit bytes remained coherently rewritable under candidate `1.2`. The current
`HCM-2.2-ESC-003` selected candidate `1.3` as the independent downstream exact
persisted result JCS+LF digest/length anchor while leaving result `1.0` and
intent `1.2` schemas unchanged. The later exact-result selector authorized the
complete bounded implementation, which proves stable locked replay, orphan and
historical-version refusal, new approval, atomic promotion/recovery, lifecycle
currentness, and selected compiler/CLI/skill/doctor/flow product paths. Fresh
implementation Review 3 required exact recomputation of candidate provenance;
the complete ordered populated-leaf source bijection now derives from retained
intake plus normalized content at author, approval, promotion, persistence, and
recovery gates, with eight coherent-resigning variants proved at every runtime
seam.

**Classification:** `ContractCorrectAndProven` for the exact first-party
Charter subject; adoption is gated on fresh `CLEAN`, primary commit, and parent
closeout. This does not support a generic custom-kind intake claim.
...
| `PG-ARTIFACT-01` | a profile-selected `ArtifactInstanceDescriptor` binds a kind to path/label/requiredness/dependencies and participates in validation/doctor/flow | Phase 2 subset closed: HCM-1.4 proves descriptor-owned path/role/requiredness/applicability/capability identity, HCM-2.1 proves Project Context content authority, HCM-2.2 proves Project Authority, HCM-2.3 proves registry-brief selection and generic operations, and HCM-2.4 proves optional Environment Context, repository-selected Work Specification Stage 10 capture/handoff, Decision/Risk generic paths, and descriptor-selected aggregate flow with no fixed family/path authority. Remaining later product consumers stay open under their own slices and do not weaken the exact Phase 2 exit. |
...
| `PG-PROJ-01` | same source truth yields multiple deterministic Resolution projections with provenance | closed only for the completed HCM-3.3 private/internal engine: one byte-identical configured custom-kind source/profile/vocabulary/definition closure is replayed twice under each of two distinct admitted Resolution envelopes, with exact output/result/provenance fingerprints, unchanged source bytes/fingerprint, and both equal-output and distinct-output cross-envelope accounting. Public API/default catalog/consumer/Snapshot/HCM-3.4+/Phase-3 claims remain open. |
| `PG-PROJ-02` | omitted required claims remain visible and cannot false-pass | closed only for the completed HCM-3.3 private/internal accounting wall: every rule is included, typed-omitted, or operation-mismatch not-applicable; omitted required claims cannot contribute a pass; proof effects, disclosure/support short circuits, no-read behavior, and fixed lossiness precedence are exact. Adoption, public transport, Snapshot grounding, and Phase-3 exit remain open. |
...
binds the recovered engine, focused tests/fixtures, exact source evidence, and
control-truth update. The exact adversarial valid-anchor-A/different-valid-B
negative, 32-test Context Resolution kernel, 55-test generic-lineage suite,
engine and workspace all-target/all-feature walls, workspace check, strict
Clippy, format, whitespace, handoff validator, and both self-tests are GREEN.
GitNexus scope-all and compare-to-`main` completed at `CRITICAL` with only the
...
The later exact-result selector authorized only the candidate-`1.3` HCM-2.2
implementation, which supplies bounded first-party proof for `PG-INTAKE-01`,
...
| `BR-HCM-2-PILOT-FLOW-01` | let the one-family HCM-2.1 flow cutover consume selected canonical Project Context YAML and its in-memory fixed renderer while other fixed siblings remained | deleted and accepted by P6; closed by the P7 recursive absence/behavior wall | GREEN: the bridge ID/type, mixed fixed/selected adapter, legacy Project Context output-tag exception, bridge-only tests, and residual normal-path fixed-kind authority are absent |
| `BR-HCM-2-CHARTER-FLOW-01` | bounded HCM-2.2 bridge adding selected canonical Charter YAML and its in-memory fixed renderer while other fixed siblings remained | deleted and accepted by P6; closed by the P7 recursive absence/behavior wall | GREEN: the bridge ID/type and bridge-only test are absent, while committed Charter authority and distinct source/render fingerprints remain preserved |
...
31a. Environment Context presence and validity permit advisory use but never select enforcement; future implementation authorization is task-scoped positive subset proof, feature/spec cross-target completion is separate, and sprint/orchestration aggregation is non-authoritative.
...
88. a v1.4 ancillary allowance admits only exact test fixtures, copied fixture authority, deterministic goldens, and assertions under closed path/count/changed-line and `test_proof_only` risk ceilings after fresh impact analysis; live dispatch replay recomputes additions plus deletions from the named Git baseline, closeout observations recompute them against the primary commit and must remain within the per-path, aggregate changed-line, and path-count ceilings frozen at the final clean review, any later dispatch that introduces or enlarges ancillary authority fails, and runtime/API/dependency/Cargo/version/unsafe-policy/authority/schema expansion still stops.
89. the default automatic review budget is one complete-subject discovery review or same-fingerprint burst per typed planning, implementation, proof, or final-closeout stage, one consolidated remediation pass, one different-fresh delta-focused closure review, and at most two supplemental causal remediation/closure cycles for P1/P2s directly caused or unmasked by the preceding remediation; supplemental cycles preserve selected scope/authority/risk and never reopen general discovery, while unrelated expansion or exhaustion preserves the blocker as a bounded non-completed stop, and P2 requires a demonstrated effect on the selected integrated outcome rather than an isolated redundant-defense disagreement.
90. before review, every current v1.4 parent freezes one fingerprinted registry of authorized integrated outcomes, packet IDs, and authority refs; every dispatch binds that registry and derives `causal_budget_id` from its parent orchestration and registered outcome, records an explicit monotonic stage transition and typed causal event reason, and types review as `discovery`, `closure`, or `supplemental_causal`; undeclared outcome, packet/selector/cycle/fingerprint renaming cannot reset a stage, remediation-unmasked test/proof/manifest gaps consume the next causal cycle, CLEAN cannot trigger another cycle, no more than two supplemental cycles validate, and deterministic mechanical closeout neither consumes nor resets a review cycle. A later FINDINGS review may carry an earlier P1/P2 only through optional typed `carried_finding_refs`, preserving one original owner, exact predecessor trigger, completed owner-to-carrier remediation, and the next cycle's exact owned-plus-carried trigger array.
91. before the first discovery review in each stage, the v1.4 dispatch records the complete applicable packet wall, recursive fixture/consumer inventory, manifest replay, formatting, and whitespace checks, then is schema-validated and replayed live with `uv run --with jsonschema==4.25.1 python docs/specs/handbook-contract-membrane/handoffs/validate_handoffs.py --verify-dispatch <repo-relative-dispatch-path>`.
92. every v1.4 parent handoff mechanically reconciles `delegated_runs` against the complete immutable dispatch population for that parent through the handoff time, including completed, failed, blocked, abandoned, and deliberately non-executed dispatches; all timestamps are canonical UTC seconds with `Z` and ordering uses parsed instants; missing or extra dispatches, future-dated population entries, count/budget drift, aggregate-fingerprint mismatch, ancillary observation drift, or any byte/count change to the frozen 66-dispatch/12-record v1.3 corpus fails closed.
...
- the explicit user decision record fixes exactly six first-party kind refs,
  exactly three root-profile instance descriptors, always/always/optional
  requiredness with no Environment Context condition,
...
- the originally published managed-operational-surface condition was an exact
  typed fingerprint producer with no evaluator; the HCM-2.4 greenfield
  correction removes it from the active product rather than promoting it to a
  runtime authority. The empty shipped vocabulary, four-level/six-domain
  Context Resolution stack, and matcher/escalation/promotion records remain
  exact typed fingerprint producers only;
...
that completed bounded HCM-2.1 implementation. At its closeout, HCM-2.2
remained separately gated; the next section records the later selected result.
...
Escalation `HCM-2.2-ESC-002` selected bounded atomic whole-file staging. Each
canonical-new, promotion-record-new, and lifecycle-transition-new output is
first written, fsynced, closed, bounded/no-follow reopened, and byte-for-byte
verified in independent engine-random create-new sibling `.output-staging/`
scratch, then atomically rename-no-replace published and directory-fsynced.
Authoritative new-output stages are therefore absent or exact; scratch is never
recovery authority. That planning subject was review-clean, but its later
implementation was not.

Fresh implementation Review 3 admitted the exact 199-path subject at baseline
`486458acfe8373a977e595a4854ac786166e3e76` and returned Required finding
`HCM-2.2-AR3-001`. The coherent-rewrite RED changed only audit-only
`validated_at_utc`, wrote the changed exact bytes equally to result/witness,
recomputed the mutable sidecar binding, and observed replay acceptance with
unchanged result semantic and candidate `1.2` identities. The complete subject
is rejected and preserved as evidence.

User-selected `HCM-2.2-ESC-003` adds candidate `1.3` as the independent
downstream exact-result authority. The candidate-subject fingerprint remains
complete and excludes result binding. Result `1.0` keeps binding only that
subject fingerprint. Final candidate `1.3` contains one closed binding with
semantic result ref/fingerprint and exact persisted result JCS+LF SHA-256/
LF-inclusive byte length. New approval binds final candidate, and promotion/
intent `1.2` bind it transitively. The graph is strictly acyclic; result
self-hash and mutable witness/binding/receipt/sidecar authority are prohibited.

Freeze-author replay independently recomputes subject and expected semantic
result authority without `validated_at_utc`, inventories relevant stores
no-follow, and discovers by their pair. Zero/no-result alone permits first
authoring; exactly one safe exact match permits read-only replay; more than one,
missing result, orphan result, unsafe/crossed/mismatching authority, or wrong
digest/length refuses before mutation and preserves all evidence. Automatic
orphan adoption or crash completion is absent; it would require a separately
reviewed author-publication transaction.

The inventory is now mechanically bounded per exact candidate/result root at
4,096 immediate entries, 262,144 bytes per regular file, 1,073,741,824
aggregate bytes, and 128 UTF-8 filename bytes, with exact 79/97-byte ASCII
grammars. Two complete no-follow sorted scans under the author lock must have
identical root and raw-name/file-identity/length/SHA-256 tuple vectors. Every
overflow, unsafe/unowned entry, duplicate-key/schema failure, or scan change
refuses before mutation; safe historical/unrelated entries remain retained
nonmatches. The selected create and amendment chains use fresh assertions,
approvals, promotions, and amendment clearance transition, and the intent
resolves their exact records rather than carrying candidate-`1.2` authority.

Candidate `1.0`, `1.1`, and `1.2`, their approvals, released records, lifecycle
heads, the rejected implementation subject, and the verified recoverable snapshot
remain immutable evidence. Candidate `1.3` must be re-authored with a newly
computed result and new approvals; there is no dual read, automatic upgrade,
result-ref copy, approval carry-forward, or selected-product fallback. Result
...
used the additive `1.1` field set for base `1.0` and omitted stored
`candidate_id` equality. The repaired validator now admits distinct exact-
closed `1.0`, `1.1`, and `1.2` shapes; validates their refs, fingerprints,
...
structurally closed but not reconstructed from their retained semantic
authority, so an attacker could coherently resign the candidate/result/
approval/promotion chain around forged provenance. The repaired central gate
derives the exact ordered sixteen-coverage, 113-populated-leaf source bijection
from retained intake plus normalized content and requires exact equality before
...
- every released definition byte remains unchanged; the additive `1.1.0`
  ...
  approval, waiver, trigger, and semantic-validator definitions reproduce
  their exact refs/fingerprints and package closure;
- one engine-owned canonical Charter parser/emitter/renderer provides exact
  source/render fingerprint domains, duplicate/multi-document/unknown/bounds/
  capability negatives, and deterministic full-byte YAML and Markdown goldens;
- all three explicit acquisition modes evaluate the same 16-coverage intake
  definition and candidate schema, preserve every leaf source and typed
  provenance class, expose unknown/contradictory/waived coverage, and never
  infer normative authority silently;
- immutable intake/candidate/approval/promotion/lifecycle records retain exact
  create/amend basis and fingerprint equality; candidate `1.3` alone carries
  the closed exact-result binding while result `1.0` binds only its complete
  subject fingerprint, approval binds its final fingerprint, and promotion/
  intent `1.2` bind it transitively; zero/one/many replay discovery, orphan,
  missing-result, wrong digest/length, semantic/raw crossing, unsafe-entry,
  candidate-`1.2` fallback, approval carry-forward, and refusal-preserves-
  evidence negatives fail closed; native CTAP2.1 registration and assertion
  verification, quorum, approval use, registry mutation, stale/forged/ABA,
  final-use, and lifecycle reassessment matrices also fail closed;
...
- setup alone initializes or preserves the strict no-follow, durable repository
  ...
  remain equal through request, native challenge, journal, result, and output;
- compiler/CLI expose only the frozen author/approve/promote/validate and
  approver-admin grammar, project exact engine-owned JSON/human envelopes, and
  refuse missing identity, entropy, unsafe recovery, missing selected truth,
  or unavailable native authentication before any current-operation delta;
- generated and installed skills use explicit guided-adaptive acquisition,
  immutable candidate replay, selected-truth validation, and native authority
  without a nested model/questionnaire, credential variable, canonical write,
  persistent Markdown, or misplaced run evidence;
- setup/doctor `1.2.0`, Environment Inventory's selected Charter reference,
  and `BR-HCM-2-CHARTER-FLOW-01` consume retained selected truth; flow/compiler
  C04 is `reduced-v1-m8.3`, C03 remains `reduced-v1-m8` generation `1`, and
  legacy Charter input/Markdown has zero selected influence;
- native Windows read-only and fail-before-mutation tests, full workspace
...
HCM-2.3 completed the exact repository-defined `registry-brief` path at
implementation commit `628b672ef33326e87e4fb30be13489e8af04b38c`.
The final implementation proof is
[`slices/HCM-2.3/proof/20260725T152135Z--59-surface-successor-implementation-proof-wall.md`](slices/HCM-2.3/proof/20260725T152135Z--59-surface-successor-implementation-proof-wall.md).
Final complete-subject review commit
`746fff667f6fbe0270182a467285d60394362530` preserves
...
which binds the exact 111-path subject
`sha256:d510e94e5b47db209022927bc4afa74b8820e27cd645123cc1cdc3aa5f72fdef`.
Checkpoint C, the full proof wall, GitNexus detection, and the complete-subject
review all passed. Commit
`7b3b5454ef363d08e4c6c78e6a201d8c36c10c5c` establishes
`handbook-engine` `0.2.0` as the accepted compatibility boundary for the
landed public-enum additions; this is not a tag, publication, or downstream
adoption claim.

The landed proof establishes:
...
The exact HCM-2.3 registry-brief subsets of `Artifact kind/schema registry` and
`Charter intake coverage` therefore move `TargetOnly -> RealPathAdopted`.
`PG-KIND-01` and `PG-ARTIFACT-01` gain only the exact subset evidence stated
above, and `PG-KIND-02` closes only for the proven registry-brief path.
First-party Charter remains `ContractCorrectAndProven`; broader
...
## HCM-2.4 completed evidence boundary
...
rewriting their immutable proof records. The corrected boundary consists of:
...
- P4/P5 token commit `00dde0162fcb15576c83b6ed40ab7286488d0890`,
  whose different-fresh supplemental causal reviewer
  `/root/hcm_2_4_p4_p5_token_impl_supplemental_1` returned CLEAN over the exact
  17-path subject
  `sha256:00c19024eeb94a2e93263489fd255f0ea47c3730bfc34da2e9cf6a054de1e5b9`;
  only `artifact_mutation::intake_commit_plan` changed in production, replacing
...
the active implementation-contract acceptance gate. The accepted P6 repair
operates under
`slices/HCM-2.4/decision/20260730-p6-protocol-integrity-repair.md`. It preserves
primary commit `1209430240eb67c7aa2baac5859d84cb9a84a3b2` and closeout
`4aecd1b42356592ccd4d093c85486a9c25410dca`, removes both bridge IDs, all four
bridge types, and residual normal-path fixed-kind authority, and retains exact
descriptor-owned identity and bytes. CLEAN in
`handoffs/dispatches/20260731T004500Z--HCM-2-4--p6-final-review-control-truth-supplemental-2-closure.json`
accepted P6. The operator explicitly selected P7. Its proof record
`slices/HCM-2.4/proof/implementation/P7-phase-2-exit-closeout.md` maps and
positively proves all eight Phase 2 exit rows, replays every P1A-P6 family,
passes the serial all-features workspace, strict lint, package/archive,
document, handoff, absence, and exact staged GitNexus walls, records the
compare-to-`main` Windows-backend diagnostic, and registers both surviving P3
advisories. Discovery dispatch
`handoffs/dispatches/20260731T022800Z--HCM-2-4--p7-phase-2-exit-final-review.json`
returned P2 `HCM-P7-FR-0001` and `HCM-P7-FR-0002`; the parent corrected only
authorized control-pack/proof text. The first remediated closure
`handoffs/dispatches/20260731T024500Z--HCM-2-4--p7-phase-2-exit-remediation-closure.json`
closed `HCM-P7-FR-0001` but returned FINDINGS because identical staged
GitNexus replays reported 10, 12, and 14 documentation symbols. The stable
result is the exact authorized path set, zero affected processes, and LOW risk;
symbol cardinality is diagnostic. The supplemental closure
`handoffs/dispatches/20260731T025730Z--HCM-2-4--p7-phase-2-exit-gitnexus-supplemental-closure.json`
returned FINDINGS because `HCM-P7-FR-0003` proved the old parent's reused
finding ownership cannot produce a truthful completed v1.4 handoff. Decision
`slices/HCM-2.4/decision/20260731-p7-final-review-lineage-repair.md` makes all
three old dispatches immutable context only and authorizes a distinct parent,
registry, and budget without relaxing validation. CLEAN in
