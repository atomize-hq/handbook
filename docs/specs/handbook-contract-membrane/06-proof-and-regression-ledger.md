...
89. the default automatic review budget is one complete-subject discovery review or same-fingerprint burst, one consolidated remediation pass, one different-fresh delta-focused closure review, and at most two supplemental causal remediation/closure cycles for P1/P2s directly caused or unmasked by the preceding remediation; supplemental cycles preserve selected scope/authority/risk and never reopen general discovery, while unrelated expansion or exhaustion preserves the blocker as a bounded non-completed stop, and P2 requires a demonstrated effect on the selected integrated outcome rather than an isolated redundant-defense disagreement.
90. every current review dispatch types its cycle as `discovery`, `closure`, or `supplemental_causal`; closure and supplemental triggers name exactly the immediately preceding findings runs and P1/P2 IDs, typed remediation must cross into the immediately following cycle with a changed subject fingerprint rather than a CLEAN peer in the same burst or unchanged adjacent cycle, CLEAN cannot trigger another cycle, no more than two supplemental cycles validate, and deterministic mechanical closeout neither consumes nor resets a review cycle.
91. before the first agent in every review cycle is spawned, the current v1.3 dispatch is schema-validated and replayed against the live manifest with `uv run --with jsonschema==4.25.1 python docs/specs/handbook-contract-membrane/handoffs/validate_handoffs.py --verify-dispatch <repo-relative-dispatch-path>`; every UTF-8 text entry is checked for trailing spaces or tabs, so subject hygiene covers the complete declared subject rather than only the files noticed during review, while v1.1/v1.2 remain exact immutable predecessor evidence and cannot be executed or used by a new closeout.
