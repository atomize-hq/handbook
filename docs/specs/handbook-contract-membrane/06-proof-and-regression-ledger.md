...
| `PG-PROJ-01` | same source truth yields multiple deterministic Resolution projections with provenance | closed only for the selected HCM-3.3 private/internal engine: one byte-identical configured custom-kind source/profile/vocabulary/definition closure is replayed twice under each of two distinct admitted Resolution envelopes, with exact output/result/provenance fingerprints, unchanged source bytes/fingerprint, and both equal-output and distinct-output cross-envelope accounting. Public API/default catalog/consumer/Snapshot/HCM-3.4+/Phase-3 claims remain open. |
| `PG-PROJ-02` | omitted required claims remain visible and cannot false-pass | closed only for the selected HCM-3.3 private/internal accounting wall: every rule is included, typed-omitted, or operation-mismatch not-applicable; omitted required claims cannot contribute a pass; proof effects, disclosure/support short circuits, no-read behavior, and fixed lossiness precedence are exact. Adoption, public transport, Snapshot grounding, and Phase-3 exit remain open. |
