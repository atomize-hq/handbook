use crate::profile_readiness::RepositoryReadinessStatus;
#[cfg(not(unix))]
use crate::setup::run_setup;
use crate::setup::{
    plan_setup, plan_setup_with_decisions, run_setup_with_decisions, SetupArtifactActionKind,
    SetupErrorCode, SetupErrorKind, SetupErrorReasonCode, SetupMode, SetupRequest, SetupRootAction,
};
use handbook_engine::{
...
use crate::doctor::doctor_with_decisions;
#[cfg(unix)]
use handbook_engine::{
...
fn assert_setup_doctor_status_and_actions(
    ...
    let before_doctor = observed_paths
        .iter()
        .map(|relative| (relative, fs::read(repo.join(relative)).ok()))
        .collect::<Vec<_>>();
    let doctor = doctor_with_decisions(repo, decisions).unwrap();

    for (relative, before) in before_doctor {
        assert_eq!(
            fs::read(repo.join(relative)).ok(),
            before,
            "doctor must not mutate {relative}"
        );
    }
