use crate::author_route_api::ApproverMapping;
use handbook_engine::{
...
pub struct CharterOperationResult {
    ...
    pub invocation_failure: Option<RepositoryInvocationFailureView>,
}

/// Closed projection of the repository-invocation refusal. It keeps the
/// engine record private while permitting the CLI to retain its legacy JSON
/// response shape.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct RepositoryInvocationFailureView {
    pub schema_id: String,
    pub schema_version: String,
    pub operation: String,
    pub stage: Option<String>,
    pub status: String,
    pub repository_identity_fingerprint: Option<String>,
    pub operation_id: Option<String>,
    pub changed_paths: Vec<String>,
    pub refusal: AdapterRefusal,
    pub next_actions: Vec<String>,
}

#[derive(Clone, Debug, PartialEq)]
pub(crate) enum CharterCommandIntent {
    Author {
...
pub(crate) fn execute_charter_command(
...
fn repository_invocation_failure(
    ...
    CharterOperationResult {
        ...
        invocation_failure: Some(repository_invocation_failure_view(&failure)),
    }
}

fn repository_invocation_failure_view(
    failure: &RepositoryInvocationPreparationFailureV1,
) -> RepositoryInvocationFailureView {
    let document = serde_json::to_value(failure)
        .expect("engine repository-invocation failure must remain serializable internally");
    let refusal = failure.refusal();
    RepositoryInvocationFailureView {
        schema_id: failure.schema_id().to_owned(),
        schema_version: failure.schema_version().to_owned(),
        operation: document["operation"]
            .as_str()
            .expect("engine failure operation")
            .to_owned(),
        stage: failure.stage().map(ToOwned::to_owned),
        status: failure.status().to_owned(),
        repository_identity_fingerprint: document["repository_identity_fingerprint"]
            .as_str()
            .map(ToOwned::to_owned),
        operation_id: document["operation_id"].as_str().map(ToOwned::to_owned),
        changed_paths: failure.changed_paths().to_vec(),
...
pub(crate) fn legacy_charter_input_refusal() -> CharterOperationResult {
    refused_charter(
        CharterOperation::Author,
...
pub(crate) fn invalid_charter_command_refusal(
    operation: CharterOperation,
) -> CharterOperationResult {
    refused_charter(
        operation,
...
pub(crate) fn invalid_intake_envelope_refusal() -> CharterOperationResult {
    refused_charter(
        CharterOperation::Author,
    ...
    fn injected_native_port_reaches_engine_with_engine_allocated_admin_identity() {
        ...
        let result = execute_approver_admin_intent_with_port(
            ...
            ApproverAdminIntent::Bootstrap {
                initial_charter_quorum: vec![ApproverMapping::new(
                    "charter_approval",
                    "repository_charter_approver",
                )],
            },
            port,
        );
        ...
        assert_eq!(result.schema_id, "handbook.approver-admin-result");
        assert_eq!(result.operation, ApproverAdminOperation::Bootstrap);
        assert_eq!(
            result
                .legacy_projection
                .as_ref()
                .and_then(|projection| match projection {
                    ApproverLegacyProjection::Admin(fields) => {
                        Some(fields.repository_identity_fingerprint.as_str())
                    }
                    ApproverLegacyProjection::InvocationFailure(_) => None,
                }),
            Some(initialized.repository_identity_fingerprint())
        );
        assert!(result
            .legacy_projection
            .as_ref()
            .and_then(|projection| match projection {
                ApproverLegacyProjection::Admin(fields) => Some(fields.operation_id.as_str()),
                ApproverLegacyProjection::InvocationFailure(_) => None,
            })
        ...
        assert_eq!(
            ...
            Some("AUTHENTICATOR_UNAVAILABLE")
        );
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub(crate) enum ApproverAdminIntent {
    Bootstrap {
...
/// Closed projection of the legacy approver-admin result. The engine record
/// stays internal; the CLI alone turns these typed fields into legacy JSON.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct ApproverAdminLegacyFields {
    pub operation_id: String,
    pub repository_identity_fingerprint: String,
    pub prior_registry_state_ref: Option<String>,
    pub prior_registry_state_fingerprint: Option<String>,
    pub result_registry_state_ref: Option<String>,
    pub result_registry_state_fingerprint: Option<String>,
    pub transition_ref: Option<String>,
    pub transition_fingerprint: Option<String>,
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub enum ApproverLegacyProjection {
    Admin(ApproverAdminLegacyFields),
    InvocationFailure(RepositoryInvocationFailureView),
}
...
pub struct ApproverAdapterResult {
    ...
    pub refusal: Option<AdapterRefusal>,
    pub next_actions: Vec<String>,
    pub legacy_projection: Option<ApproverLegacyProjection>,
}

pub(crate) fn execute_approver_admin_intent(
    repo_root: impl AsRef<Path>,
...
fn execute_approver_admin_intent_with_port<P: NativeAuthenticatorPortV1>(
    ...
    let (operation, identity_operation, request_operation, mut document) = match intent {
        ...
        } => match mapping_values(initial_charter_quorum) {
        ...
        ApproverAdminIntent::AddCredential { approval_mappings } => {
            match mapping_values(approval_mappings) {
...
fn mapping_values(values: Vec<ApproverMapping>) -> Result<Vec<Value>, String> {
    let mut mappings = values
        .into_iter()
        .map(|value| {
            if value.approval_class().is_empty()
                || value.authority_ref().is_empty()
                || value.approval_class().contains('\0')
                || value.authority_ref().contains('\0')
            {
                return Err(
...
fn approver_engine_result(
    ...
    ApproverAdapterResult {
        ...
        legacy_projection: Some(ApproverLegacyProjection::Admin(
            approver_admin_legacy_fields(&value),
        )),
    }
}
...
fn approver_invocation_failure(
    ...
    ApproverAdapterResult {
        ...
        next_actions: failure.next_actions().to_vec(),
        legacy_projection: Some(ApproverLegacyProjection::InvocationFailure(
            repository_invocation_failure_view(&failure),
        )),
    }
}
...
fn approver_admin_legacy_fields(value: &Value) -> ApproverAdminLegacyFields {
    ApproverAdminLegacyFields {
        ...
        repository_identity_fingerprint: value["repository_identity_fingerprint"]
            .as_str()
            .expect("engine repository identity fingerprint")
            .to_owned(),
        prior_registry_state_ref: optional_string(value, "prior_registry_state_ref"),
        prior_registry_state_fingerprint: optional_string(
            value,
            "prior_registry_state_fingerprint",
        ),
        result_registry_state_ref: optional_string(value, "result_registry_state_ref"),
        result_registry_state_fingerprint: optional_string(
            value,
            "result_registry_state_fingerprint",
        ),
        transition_ref: optional_string(value, "transition_ref"),
        transition_fingerprint: optional_string(value, "transition_fingerprint"),
        registration_ref: optional_string(value, "registration_ref"),
        registration_fingerprint: optional_string(value, "registration_fingerprint"),
        assertion_ref: optional_string(value, "assertion_ref"),
        assertion_fingerprint: optional_string(value, "assertion_fingerprint"),
    }
}

fn optional_string(value: &Value, field: &str) -> Option<String> {
    value[field].as_str().map(ToOwned::to_owned)
}
