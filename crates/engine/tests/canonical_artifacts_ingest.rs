...
#[test]
fn descriptor_selected_collection_refuses_oversized_source_without_retaining_truncated_bytes() {
    let dir = tempfile::tempdir().expect("tempdir");
    write_file(
    ...
    assert_eq!(
        charter.render_failure.as_deref(),
        Some("document_limit_exceeded")
    );
...
#[test]
fn descriptor_selected_collection_refuses_unstable_retained_file_identity() {
    let dir = tempfile::tempdir().expect("tempdir");
    let source = dir.path().join("charter-source.yaml");
    write_file(&source, b"charter source");
    let selected = dir.path().join(".handbook/project/charter.yaml");
    std::fs::create_dir_all(selected.parent().expect("selected parent")).expect("mkdirs");
    std::fs::hard_link(&source, &selected).expect("hard link selected charter");
    write_file(
        &dir.path().join(".handbook/project/context.yaml"),
        b"project context",
    );
    ...
    assert_eq!(
        charter.render_failure.as_deref(),
        Some("observation_changed_during_inspection")
    );
    assert!(artifacts.ingest_issues.iter().any(|issue| {
        issue.kind == ArtifactIngestIssueKind::CanonicalArtifactReadError
            && issue.canonical_repo_relative_path == ".handbook/project/charter.yaml"
