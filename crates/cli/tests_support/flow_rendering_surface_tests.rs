#[cfg(unix)]
use crate::flow_rendering::render_inspect;
use crate::flow_rendering::{build_output_model, render_json, render_markdown};
#[cfg(unix)]
use handbook_engine::{
...
#[cfg(unix)]
use handbook_flow::{resolve, BudgetPolicy, ResolveRequest};
#[cfg(unix)]
...
fn json_kind_projections_remove_only_ascii_space_from_descriptor_label() {
    use crate::flow_rendering::RenderOutputModel;
    use handbook_sdk::{
        ArtifactPresence, BudgetDisposition, BudgetOutcome, BudgetReason, PacketDecisionSummary,
        PacketResult, PacketSection, PacketSectionMode, PacketSelectionStatus, PacketSourceSummary,
        PacketVariant, ReadyPacketNextSafeAction,
    };
    use handbook_sdk::{Blocker, BlockerCategory, NextSafeAction, SubjectRef};
...
fn render_markdown_keeps_trust_header_first_for_ready_result() {
    ...
    let result = HandbookSdkV1::open(root)
...
fn render_markdown_keeps_trust_header_first_for_refusal_result() {
    ...
    let result = handbook_sdk::HandbookSdkV1::open(root)
        .generate_packet(handbook_sdk::flow_api::GeneratePacketRequest::new(
            "planning.packet",
        ))
        .expect("resolve")
        .into_resolution();
