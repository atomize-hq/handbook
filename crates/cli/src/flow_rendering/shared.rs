#[cfg(all(test, unix))]
use handbook_sdk::SubjectRef;
#[cfg(test)]
use handbook_sdk::{
    Blocker, PacketBodyNote, PacketBodyNoteKind, PacketFixtureContext, PacketSection,
    PacketSectionMode, PacketSourceSummary,
};
use handbook_sdk::{BlockerCategory, NextSafeAction, RefusalCategory};
use handbook_sdk::{
    BudgetDisposition, BudgetNextSafeAction, BudgetReason, PacketResult, PacketSelectionStatus,
...
mod recovery_shell {
    use handbook_sdk::NextSafeAction;
    #[cfg(all(test, unix))]
    use handbook_sdk::SubjectRef;
